...
class Plotter:
    ...
    def MISRRLL_plot_error_time_bar(
        x,
        x_data,
        MISRRLL_avg,
        MISRRLL_diff,
        ...
        config = {
            ...
            "font.size": 9,  # 相当于小四大小
            # "font.serif": ["SimSun"],  # 宋体
            "font.serif": ["Times New Roman"],  # TNR
            "mathtext.fontset": "stix",  # matplotlib渲染数学字体时使用的字体，和Times New Roman差别不大
            "axes.unicode_minus": False,  # 处理负号，即-号
        }
        rcParams.update(config)
    ...
    def MISRRLL_plot_error_ur_box_combined(data_path, result_fig):
        """
        MISRRLL-容错-利用率-箱线图-组合
    ...
    def MISRRLL_plot_recover_time_bar(
        x,
        x_data,
        MISRRLL_avg,
        MISRRLL_diff,
        ...
        plt.xlabel(r"$\mathit{W}$", fontsize=9)
        plt.ylabel(r"$\mathrm{Recovery \; Time}$", fontsize=9)
        x_label = []
        for i in range(5, 16):
            x_label.append(str(i * 1000))
        plt.xticks(x_data, x_label)
    ...
    def MISRRLL_plot_recover_ur_box_combined(data_path, result_fig):
        """
        MISRRLL-恢复-利用率-箱线图-组合
