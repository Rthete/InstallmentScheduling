 ...
 * @LastEditTime: 2023-11-04 15:47:06
 */
#include "exp_3.h"
...
void error_TolerMIS_30_conflict() {
  ...
  for (const auto &row : error_places) {
    ...
    ModelRunner::run_MISRR_conflict(result, 30, 0, 5000, 0.3, "../data/exp1-30-servers/",
                       row, error_installment[0][0]);
    // ModelRunner::run_MISRR_conflict(result, 30, 0, 10000, 0.3, "../data/exp1-30-servers/",
    //                    row, error_installment[1][0]);
    // 对比没有冲突情况
    ModelRunner::run_MISRR_conflict(result, 30, 0, 5000, 0.3, "../data/exp1-30-servers/",
                       row, error_installment[2][0]);
    // ModelRunner::run_MISRR_conflict(result, 30, 0, 10000, 0.3, "../data/exp1-30-servers/",
...
void error_SIS_30() {
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    for (int w = 5000; w < 16000; w += 1000) {
      ...
      /* 每种30次取平均 */
      for (const auto &row : error_places) {
        double result = ModelRunner::run_SIS(30, w, 0.3, "../data/exp1-30-servers/", row);
        result_sum += result;
...
void error_TolerMIS_30() {
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    for (int w = 5000; w < 16000; w += 1000) {
      ...
      /* 每种30次取平均 */
      for (const auto &row : error_places) {
        double result = ModelRunner::run_MISRR(30, 0, w, 0.3, "../data/exp1-30-servers/",
...
void error_APMISRR_30() {
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    for (int w = 5000; w < 16000; w += 1000) {
      ...
      /* 每种30次取平均 */
      for (const auto &row : error_places) {
        double result = ModelRunner::run_myAPMISRR(30, 0.5, m[index_w], w, 0.3,
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    for (int w = 5000; w < 5500; w += 1000) {
      ...
      for (const auto &row : error_places) {
        double result = ModelRunner::run_SIS(30, w, 0.3, "../data/exp1-30-servers/", row);
        singleFile << result << ",";
        result = ModelRunner::run_MISRR(30, 0, w, 0.3, "../data/exp1-30-servers/", row,
                           error_installment[index_w][index_30]);
        singleFile << result << ",";
        result = ModelRunner::run_myAPMISRR(30, 0.5, m[index_w], w, 0.3,
                               "../data/exp1-30-servers/", row,
                               error_installment[index_w][index_30]);
        singleFile << result << "\n";
        index_30++;
